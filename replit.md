...
**Vehicle Management UI**: Admin vehicle configuration page (`/fahrzeuge`) provides edit dialog for atomic vehicle name and funkrufname updates. Dialog uses PATCH /api/vehicle-configs/:id/complete endpoint with optimistic UI updates and automatic cache invalidation for both `/api/vehicles` and `/api/vehicle-configs` queries. Handles rename collisions with user-friendly error messages.
...
**API Design**: RESTful API endpoints under `/api` for authentication, user management (including qualifications, roles, password reset), vehicle management (with automatic default configuration creation), qualification management, mission info, system settings, staffing verification, calendar events (CRUD, RSVP, export), vehicle configurations (CRUD, import/export), **atomic vehicle/config updates** (PATCH /api/vehicle-configs/:id/complete for transactional name/funkrufname changes with collision detection and auto-create/adopt logic), automatic crew assignment, **push notification management** (subscribe, unsubscribe, VAPID public key), **availability templates** (CRUD, apply to week), **reminder settings** (GET, PATCH), and **alarm integration** (incoming webhook, event history, simulation endpoint).
**Data Layer**: Uses an in-memory storage implementation (`MemStorage`) designed to support database persistence through an `IStorage` interface. **Storage Parity**: Both `MemStorage` (dev) and `PostgresStorage` (prod) implement identical three-path logic for vehicle/config updates: (1) missing current vehicle → adopt existing target or auto-create, (2) rename with collision detection → prevent duplicate vehicles, (3) no-op updates → preserve existing data. Enforces `config.vehicle === vehicles.name` invariant across all update paths. PostgreSQL implementation uses transactions for atomicity.
...
**Fairness/Rotation System**: FairnessScorer class implements position rotation with configurable weights. Tracks assignment history and metrics per user. Scoring combines qualification fit, recency penalty (0-10+ points), scarcity bonus (1.5-3 points), and position weights (Maschinist=3.0, Führung=2.0, Standard=1.0). Batch-loads user data for performance (O(users) instead of O(users × slots) queries). Automatic history tracking after each assignment. Configurable rotation window (default 4 weeks) in settings. **Reassignment tracking**: When users are moved from support to tactical vehicles, fairness history is updated (not duplicated) to reflect final assignment position.
**Mission-Type-Aware Crew Assignment**: Three-tier multi-pass algorithm ensures tactical vehicles fill first:
  - Priority 1 (Tactical Fire/Rescue): LF, HLF, TLF (9 slots each)
  - Priority 2 (Specialized): DL, RW (3 slots each)
  - Priority 3 (Support/Command): MTF, ELW (6 slots)
  After initial assignment, **intelligent reassignment phase** moves qualified personnel from fully-staffed support vehicles to tactical vehicles with empty critical positions (GF, MA, AT). Mission type (brandeinsatz, technische_hilfeleistung, gefahrgut, standard) adjusts vehicle priorities dynamically. Prevents scenario where qualified personnel sit idle on MTF/ELW while LF positions remain empty. **Support Vehicle Staffing Protection**: Support vehicles (MTF, ELW, KdoW) maintain minimum 2-person crew during reassignment. Specialized vehicles (RW, GWG, Wasserträger, SW, DL, DLK) are completely protected from being used as reassignment donors. Triple-check logic with defensive logging ensures critical vehicles remain operational.
...
- **vehicle_priorities**: Mission-specific vehicle priority configuration (vehicle_type unique, display_name, brandeinsatz_priority, technische_hilfeleistung_priority, gefahrgut_priority, standard_priority, beschreibung). Supports 9 vehicle types with FwDV-based priority tiers.
